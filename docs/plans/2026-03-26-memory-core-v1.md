...
- 记忆融合策略（先做根因修复）
  - 在 `MemoryManager` 引入 `FusionConfig`（默认 `mode="rank"`、`overfetch_factor=3`、`store_weights` 默认全 `1.0`）。
...
- 本地持久化与恢复语义（最小可用）
  ...
  - `stores` 内每个后端保存 `MemoryItem` 全字段；图存储额外保存边列表（含 relation attrs）；情景存储额外保存 timeline。
  - `load_snapshot` 在 `strict=True` 时执行确定性校验：schema 版本、必填字段、ID 唯一性、graph 边端点存在性，不通过则抛错并拒绝部分加载。
- 可重复评测与产物输出
  - 新增 `src/memory/evaluation.py`（CLI）：固定 seed 构造小型场景，执行检索与 roundtrip。
