...
工程 blog 中还出现了一类更轻量、但很值得注意的结构化检索思路：先把长文档或长期上下文组织成层次化摘要树，再在查询时自顶向下逐层路由到更具体的节点。它不依赖 embedding，相当于把“目录 -> 章节 -> 小节 -> 原文”这一路径显式建模。这个方向说明，retrieval 不一定只能理解为 ANN top-k，也可以是可解释的层次化导航过程。
