...
- `src/memory/evaluation.py` 现有原型评测覆盖 formation / evolution / retrieval / roundtrip，属于可重复的最小闭环。
- `ref/datasets/` 中已下载的数据集可以归一化成统一 case 格式，支持对 LoCoMo、LongMemEval、MemoryAgentBench、MemoryArena、AMA-Bench 做同一接口下的比较。
- 如果某个数据集只提供 question/answer pair，它仍然只应被视为 retrieval hit 层，不应被描述成完整的 formation 或 evolution benchmark。
- 如果数据集包含显式更新、遗忘或状态迁移字段，才可以把它纳入 evolution 或 memory-in-use 口径。
