...
> v3.3.0 | 2026-03-30
> Changelog: 新增"因果记忆"作为更远 frontier 方向。
...
一个尚未在当前文献中被系统处理但逻辑上不可回避的方向是因果记忆（causal memory）：agent 不仅记住"发生了什么"（episodic）和"世界是什么样"（semantic），还需要记住"什么导致了什么"（causal）。当 agent 需要解释行动失败、避免重复错误、或在新情境中泛化旧经验时，因果结构是不可替代的——纯粹的时序共现或语义相似度无法区分因果关系和相关关系。当前 Hindsight 的 belief-aware 分层隐含了因果方向（evidence → belief → action），但没有显式建模因果图。这是一个证据尚不充分但方向确定的前沿。
