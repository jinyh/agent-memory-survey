...
其中，MEM 额外强调了一种很有代表性的 formation 分层：短期保留高密度视觉观察，长期保留更可压缩的语言化任务进展与环境状态。这说明 formation 不只是“写不写入”，还包括“同一经历在不同时间尺度上写成什么表示”。
...
- `TeleMem / MEM / MemOCR / M3-Agent`：代表多模态 formation 开始从文本事实扩展到片段、对象和场景状态；其中 MEM 进一步强调短期观察与长期任务状态的分层写入。

## 本章主要证据来源

- `paper`：Mem0、Agentic Memory、Memoria、MemSkill、MemAgent、TeleMem、MEM、MemOCR、M3-Agent 相关论文与索引条目。
