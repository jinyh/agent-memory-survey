...
make test    # 运行所有测试
make lint    # ruff 代码检查
make refs    # 重建 references 索引
make eval    # 全生命周期评测
make agent   # 运行 agent 示例
make docs    # 检查文档内链（scripts/check-doc-links.py）
```
...
uv run --active --extra dev pytest tests/test_memory.py -q        # 最常用的记忆层回归
...
这个仓库不是 code-first 开发流，而是文档驱动的研究闭环，默认按以下 6 步推进：
...
- `export_memoryagentbench.py`：将评测结果导出为 MemoryAgentBench 格式
